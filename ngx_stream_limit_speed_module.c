...
//##########  Limiter Define Start   ##########
#define MAX_FD              10240
#define MAX_key2fd_member   10000
#define Worker_Process      32
//##########  Limiter Define End     ##########


//##########  Limiter Struct Start   ##########
//Data结构
...
typedef struct
{
    unsigned int   pids[Worker_Process];
    unsigned int   u_speed[Worker_Process][MAX_FD];         // 存储每个fd的上行速度
    unsigned int   d_speed[Worker_Process][MAX_FD];         // 存储每个fd的下行速度

    ngx_rbtree_t        *tree;                              // 红黑树，用于存储每个key的hash对应的所有fd，tree的每一个节点通
                                                            // 过链表来存储有效信息，即 node->data->fdl 存储了所有fd
...
//##########  Limiter Function Start   ##########
// Command Functions
static char * ngx_minium_speed(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
static char * ngx_rank0_speed(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
static char * ngx_rank1_speed(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
static char * ngx_rank2_speed(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
static char * ngx_rank3_speed(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
static char * ngx_redis_pwd(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
static char * ngx_redis_conf(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
static char * ngx_limiter_disable(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);

//Helper Functions
static ngx_int_t ngx_stream_limit_speed_init(ngx_conf_t *cf);
static void ngx_string_rbtree_insert_value(ngx_rbtree_node_t *temp, ngx_rbtree_node_t *node, ngx_rbtree_node_t *sentinel);
static ngx_duplex_node_t * ngx_find_value(ngx_rbtree_t *r, uint32_t hash);
static ngx_int_t ngx_stream_init_shm_zone(ngx_shm_zone_t *shm_zone, void *data);
void travel_rbtree(ngx_rbtree_node_t *root, ngx_rbtree_node_t *sentinel);
//##########  Limiter Function End     ##########
...
static ngx_command_t  ngx_stream_proxy_commands[] = {
    ...
    { ngx_string("limiter_minium_speed"),
      NGX_STREAM_MAIN_CONF | NGX_STREAM_SRV_CONF | NGX_CONF_TAKE1,
      ngx_minium_speed,
      NGX_STREAM_SRV_CONF_OFFSET,
      0,
    ...
    { ngx_string("limiter_rank0_speed"),
    ...
    { ngx_string("limiter_rank1_speed"),
      NGX_STREAM_MAIN_CONF | NGX_STREAM_SRV_CONF | NGX_CONF_TAKE2,
      ngx_rank1_speed,
      NGX_STREAM_SRV_CONF_OFFSET,
      0,
    ...
    { ngx_string("limiter_rank2_speed"),
      NGX_STREAM_MAIN_CONF | NGX_STREAM_SRV_CONF | NGX_CONF_TAKE2,
      ngx_rank2_speed,
      NGX_STREAM_SRV_CONF_OFFSET,
      0,
    ...
    { ngx_string("limiter_rank3_speed"),
      NGX_STREAM_MAIN_CONF | NGX_STREAM_SRV_CONF | NGX_CONF_TAKE2,
      ngx_rank3_speed,
      NGX_STREAM_SRV_CONF_OFFSET,
      0,
...
static ngx_stream_module_t  ngx_stream_proxy_module_ctx = {
    ...
    ngx_stream_limit_speed_init,           /* postconfiguration */
...
ngx_module_t  ngx_stream_limit_speed_module = {
    NGX_MODULE_V1,
...
//##########  Limiter Variable Start   ##########
static eredis_t       *eredis_conn = NULL;
static ngx_str_t      eredis_pwd = (ngx_str_t) ngx_null_string;
static ngx_uint_t     x_limit_min_speed = 1024 ;    //默认限速
static char           cmd_ping[4] = "PING";         //ping 命令
static char           redis_conf_loc[1024] = {0};
static ngx_shm_zone_t *shm_zone = NULL;

static uint32_t       disable_switch = 0;

#if (NGX_PCRE)
ngx_regex_compile_t   arg_regex_rc;
u_char                arg_regex_errstr[NGX_MAX_CONF_ERRSTR];
#endif

static unsigned int   fd_flag[MAX_FD] = {0};        // 临时变量，记录某个fd是否已经经过限速流程（无论key是否有效）
static unsigned int   fd_hash[MAX_FD] = {0};        // 存储每个fd对应的key的hash
static unsigned int   fd_time[2][MAX_FD] = {{0}};        // 存储每个fd对应的上一次判定传输
static unsigned int   fd_trans[2][MAX_FD] = {{0}};       // 存储每个fd对应的上一次判定传输

static int            mem_pid = -1;                 // 存储每个worker的pid序号

static unsigned int   u_rank[16]    = {0};
static unsigned int   d_rank[16]    = {0};

//##########  Limiter Variable Ends    ##########
...
ngx_stream_proxy_handler(ngx_stream_session_t *s)
{
    ...
    pscf = ngx_stream_get_module_srv_conf(s, ngx_stream_limit_speed_module);
...
ngx_stream_proxy_init_upstream(ngx_stream_session_t *s)
{
    ...
    if (c->buffer && c->buffer->pos < c->buffer->last) {
        ...
        *cl->buf = *c->buffer;
        ...
        cl->buf->tag = (ngx_buf_tag_t) &ngx_stream_limit_speed_module;
        cl->buf->flush = 1;
    ...
    ngx_speed_control_t         *shm_control = (ngx_speed_control_t *)shm_zone->data;
    ...
    if (mem_pid == -1)
    {
        ...
        {
            if (shm_control->pids[i] == 0)
            {
                shm_control->pids[i] = getpid();
                mem_pid = i;
                break;
            }
        }
    ...
    ngx_shmtx_lock(&shpool->mutex);
    shm_control->d_speed[mem_pid][s->connection->fd] = x_limit_min_speed;
    shm_control->u_speed[mem_pid][s->connection->fd] = x_limit_min_speed;
    ngx_shmtx_unlock(&shpool->mutex);

    fd_time[0][s->connection->fd] = s->upstream->start_sec;
    fd_time[1][s->connection->fd] = s->upstream->start_sec;
    fd_trans[0][s->connection->fd] = 0;
    fd_trans[1][s->connection->fd] = 0;
    ...
    ngx_uint_t do_write)
{
    ...
    int                           captures[(1 + 2) * 3];
    char                          buffer[1024] = {0};
    //char                          redis_buff[1024] = {0};
    ngx_str_t                     input;
    int                           stream_index = 0;
    char                         *i;
    uint32_t                      hash;
    ngx_duplex_node_t            *tmp = NULL;
    ngx_duplex_node_t            *nodeToInsert;
    fd_list_t                    *tmp_fd_node;
    char                          cmds[100] = { 0 };
    ngx_speed_control_t          *shm_control = (ngx_speed_control_t *) shm_zone->data;
    ngx_slab_pool_t              *shpool = (ngx_slab_pool_t *) shm_zone->shm.addr;
    ...
    if (from_upstream) {
        ...
        stream_index = 0;
        b = &u->upstream_buf;
        limit_rate = shm_control->d_speed[mem_pid][s->connection->fd];
        received = &u->received;
        ...
        stream_index = 1;
        b = &u->downstream_buf;
        limit_rate = shm_control->u_speed[mem_pid][s->connection->fd];
        received = &s->received;
        out = &u->upstream_out;
    ...
    for ( ;; ) {
        ...
        if (do_write && dst) {
            ...
            if (*out || *busy || dst->buffered) {
                ...
                ngx_chain_update_chains(c->pool, &u->free, busy, out,
                                      (ngx_buf_tag_t) &ngx_stream_limit_speed_module);
            ...
            && !src->read->error)
        {
            if (limit_rate) {
                ...
                limit = (off_t) limit_rate * (ngx_time() - fd_time[stream_index][s->connection->fd] + 1)
                        - (*received - fd_trans[stream_index][s->connection->fd]);

                fd_time[stream_index][s->connection->fd]  = ngx_time();
                fd_trans[stream_index][s->connection->fd] = *received;
...
/* ##########  Limiter Code Start   ########## */
#if (NGX_PCRE)
            if ( fd_flag[s->connection->fd] == 0 && disable_switch == 0)
            {
                fd_flag[s->connection->fd] = 1;

                // Danger here !!!! Need to be modified
                i = strrchr((const char *)b->pos, '\n');
                if (i != NULL) {
                    if ((size_t)i > (size_t)(b->pos))
                    {
                        int len = (size_t)i - (size_t)(b->pos);
                        len = len > 1023 ? 1023 : len;
                        strncpy(testStrings, (char *)b->pos, len);
                        ngx_str_set(&input, testStrings);
                    }
                    else
                    {
                        goto done_x;
                ...
                nx = ngx_regex_exec(arg_regex_rc.regex, &input, captures, (1 + 2) * 3);
                if (nx == 1) {
                    /* String matches expression */
                    ex.len = captures[1] - captures[0];
                    ex.data = input.data + captures[0];
                    ngx_memcpy(buffer, ex.data, ex.len);

                    /* If and only if redis_conn is valid , get speed_limit from redis. */
                    if ( eredis_conn != NULL) {
                        memset((void *) cmds, 0, 100);
                        strcpy(cmds, "GET ");

                        ngx_memcpy(&cmds[4], buffer, strlen(buffer) > 32 ? 32 : strlen(buffer));
                        eredis_reply_t  *eredis_reply;
                        eredis_reader_t *eredis_reader = eredis_r( eredis_conn );
                        eredis_reply = eredis_r_cmd( eredis_reader, cmds);
                        if (eredis_reply != NULL && eredis_reply->str != NULL)
                        {
                            uint8_t tmp_rank = eredis_reply->str[0];
                            eredis_r_clear( eredis_reader );
                            eredis_r_release( eredis_reader );

                            uint8_t d_rank_index = (tmp_rank & 0xF0) >> 4;
                            uint8_t u_rank_index = tmp_rank & 0x0F;

                            {
                                hash = ngx_crc32_short((u_char *)buffer, strlen(buffer));

                                ngx_shmtx_lock(&shpool->mutex);
                                tmp = ngx_find_value(shm_control->tree, hash);
                                ngx_shmtx_unlock(&shpool->mutex);
                                if ( tmp == NULL )
                                {
                                    /* If not found, then insert */

                                    nodeToInsert = (ngx_duplex_node_t *)ngx_slab_alloc_locked(shpool, sizeof(ngx_duplex_node_t));// malloc(sizeof(ngx_duplex_node_t));
                                    if (nodeToInsert == NULL)
                                    {
                                        printf("Memory not enough.\n");
                                        goto done_x;
                                    }
                                    nodeToInsert->data.nums    = 1;
                                    nodeToInsert->node.key     = hash;
                                    nodeToInsert->data.fdl     = (fd_list_t *)ngx_slab_alloc_locked(shpool, sizeof(fd_list_t));//malloc(sizeof(fd_list_t));
                                    if (nodeToInsert->data.fdl == NULL)
                                    {
                                        printf("Memory not enough.\n");
                                        goto done_x;
                                    }
                                    nodeToInsert->data.fdl->prev = NULL;
                                    nodeToInsert->data.fdl->data = s->connection->fd;
                                    nodeToInsert->data.fdl->pid = mem_pid;
                                    nodeToInsert->data.fdl->next = NULL;

                                    ngx_shmtx_lock(&shpool->mutex);
                                    ngx_rbtree_insert(shm_control->tree, &nodeToInsert->node);
                                    shm_control->d_speed[mem_pid][s->connection->fd] = d_rank[d_rank_index];
                                    shm_control->u_speed[mem_pid][s->connection->fd] = u_rank[u_rank_index];
                                    ngx_shmtx_unlock(&shpool->mutex);
                                    fd_hash[s->connection->fd] = hash;
                                }
                                else
                                {
                                    /* If found, then update */
                                    ngx_shmtx_lock(&shpool->mutex);
                                    if (tmp->data.nums < MAX_key2fd_member)
                                    {
                                        tmp->data.nums++;
                                        tmp_fd_node = tmp->data.fdl;
                                        while (tmp_fd_node->next != NULL)
                                        {
                                            tmp_fd_node = tmp_fd_node->next;
                                        }
                                        ngx_shmtx_unlock(&shpool->mutex);
                                        tmp_fd_node->next = (fd_list_t *)ngx_slab_alloc_locked(shpool, sizeof(fd_list_t));
                                        ngx_shmtx_lock(&shpool->mutex);
                                        if (tmp_fd_node->next == NULL)
                                        {
                                            printf("Memory not enough.\n");
                                            goto done_x;
                                        }
                                        tmp_fd_node->next->prev = tmp_fd_node;
                                        tmp_fd_node->next->data = s->connection->fd;
                                        tmp_fd_node->next->pid  = mem_pid;
                                        tmp_fd_node->next->next = NULL;

                                        fd_hash[s->connection->fd] = hash;
                                    }

                                    tmp_fd_node = tmp->data.fdl;
                                    while (tmp_fd_node != NULL)
                                    {
                                        shm_control->d_speed[tmp_fd_node->pid][ tmp_fd_node->data ] = d_rank[d_rank_index] / tmp->data.nums;
                                        shm_control->u_speed[tmp_fd_node->pid][ tmp_fd_node->data ] = u_rank[u_rank_index] / tmp->data.nums;
                        ...
                        if (limit_rate < x_limit_min_speed)
                        {
                            ngx_shmtx_lock(&shpool->mutex);
                            shm_control->d_speed[mem_pid][s->connection->fd] = x_limit_min_speed;
                            shm_control->u_speed[mem_pid][s->connection->fd] = x_limit_min_speed;
                        ...
                        //ngx_log_error(NGX_LOG_WARN, r->connection->log, 0, "Redis Connection Error.");
                    }
                    
                } else if (nx == NGX_REGEX_NO_MATCHED) {
                    /* no match was found */
                } else {
                    /* some error */
                }
...
done_x:
    ...
    if (src->read->eof && dst && (dst->read->eof || !dst->buffered)) {
        ...
        ngx_log_error(NGX_LOG_INFO, c->log, 0,
                      "%s%s disconnected"
                      ", x bytes from/to client:%O/%O"
                      ", x bytes from/to upstream:%O/%O",
...
ngx_stream_proxy_finalize(ngx_stream_session_t *s, ngx_uint_t rc)
{
    ...
    //ngx_shmtx_lock(&shpool->mutex);
    tmp = ngx_find_value(shm_control->tree, fd_hash[s->connection->fd]);
    if ( tmp == NULL )
    {
        /* should to found */
    }
    else
    {
        ...
        uint32_t d_speed_tmp = shm_control->d_speed[mem_pid][s->connection->fd] * tmp->data.nums;
        uint32_t u_speed_tmp = shm_control->u_speed[mem_pid][s->connection->fd] * tmp->data.nums;
        tmp->data.nums--;
        ...
        while (tmp_fd_node->next != NULL)
        {
            if (tmp_fd_node->data == (unsigned int)s->connection->fd && (int)tmp_fd_node->pid == mem_pid)
            {
                if (tmp_fd_node->prev == NULL)
                {
                    tmp->data.fdl = tmp_fd_node->next;
                    tmp->data.fdl->prev = NULL;
                }
                else
                {
                    tmp_fd_node->prev->next = tmp_fd_node->next;
                    tmp_fd_node->next->prev = tmp_fd_node->prev;
                }
                ngx_slab_free(shpool, tmp_fd_node);
            }
            tmp_fd_node = tmp_fd_node->next;
        }
        if (tmp_fd_node->data == (unsigned int)s->connection->fd && (int)tmp_fd_node->pid == mem_pid)
        {
            if (tmp_fd_node->prev != NULL)
            {
                tmp_fd_node->prev->next = NULL;
            }
            ngx_slab_free(shpool, tmp_fd_node);
        }
        /* End Delete fd in list */
        ...
        if (tmp->data.nums > 0)
        {
            ...
            while (tmp_fd_node != NULL)
            {
                shm_control->d_speed[tmp_fd_node->pid][ tmp_fd_node->data ] = d_speed_tmp / tmp->data.nums;
                shm_control->u_speed[tmp_fd_node->pid][ tmp_fd_node->data ] = u_speed_tmp / tmp->data.nums;
                tmp_fd_node = tmp_fd_node->next;
            }
        }
        else
        {
            ngx_rbtree_delete(shm_control->tree, &tmp->node);
            ngx_slab_free(shpool, tmp);
        }
    }
    fd_flag[s->connection->fd] = 0;
    fd_hash[s->connection->fd] = 0;
...
static ngx_int_t
ngx_stream_limit_speed_init(ngx_conf_t *cf)
{
    /* Init Regex */
#if (NGX_PCRE)
    ngx_str_t             arg_regex_value = ngx_string("(?<=[lL][sS][tT][yY][pP][eE][:][\\s]).*?(?=[\\s])");
    ngx_memzero(&arg_regex_rc, sizeof(ngx_regex_compile_t));
    arg_regex_rc.pattern = arg_regex_value;
    arg_regex_rc.pool = cf->pool;
    arg_regex_rc.err.len = NGX_MAX_CONF_ERRSTR;
    arg_regex_rc.err.data = arg_regex_errstr;
    if (ngx_regex_compile(&arg_regex_rc) != NGX_OK) {
        printf("Regex Compiled Error\n");
        return NGX_ERROR;
    }
#else
    printf("PCRE Needed.\n");
    ...
    {
        if (u_rank[i] <= 0)
            u_rank[i] = x_limit_min_speed;
        if (d_rank[i] <= 0)
            d_rank[i] = x_limit_min_speed;
    }

    /* Init Redis connection  */
    eredis_conn = eredis_new();
    if (eredis_host_file( eredis_conn, redis_conf_loc ) <=0) {
        fprintf(stderr, "Unable to load conf:%s, Using Default Config 127.0.0.1:6379 .\n", redis_conf_loc);
        eredis_free( eredis_conn );
        eredis_conn = eredis_new();
        eredis_host_add( eredis_conn, "127.0.0.1", 6379 );
    }

    if (eredis_pwd.len > 0)
    {
        eredis_pc_cmd( eredis_conn, "AUTH %s", eredis_pwd.data );
        //printf("\nAUTH %s\n", eredis_pwd.data );
    }

    /* run eredis thread */
    eredis_run_thr( eredis_conn );

    /* test if redis server is valid */
    eredis_reply_t  *eredis_reply;
    eredis_reader_t *eredis_reader = eredis_r( eredis_conn );
    eredis_reply = eredis_r_cmd( eredis_reader, cmd_ping);
    if (eredis_reply == NULL)
    {
        printf("Redis Server Connection Error.\n");
    }
    else
    {
        printf("Testing:%lu\n", (long unsigned int)eredis_reply->str);
        if (ngx_strncmp(eredis_reply->str, "PONG", 4) == 0)
        {
            printf("eRedis Connect Success...\n");
        }
        else if (ngx_strncmp(eredis_reply->str, "NOAUTH", 6) == 0)
        {
            printf("eRedis Server Needs Auth.\n");
        }
        else
        {
            printf("eRedis Server Connection is invalid.\n");
        }
    }
    ...
    if (shm_zone == NULL)
    {
        ...
        shm_zone = ngx_shared_memory_add(cf, shm_name, 102400 * ngx_pagesize, &ngx_stream_limit_speed_module);
...
static ngx_duplex_node_t * 
ngx_find_value(ngx_rbtree_t *r, uint32_t hash)
{
...
{
    ngx_rbtree_node_t     **p;
    ngx_duplex_node_t     *strNodeX;
    ngx_duplex_node_t     *strNodeY;
    ...
    for ( ;; )
    {
        ...
        {
            strNodeX = (ngx_duplex_node_t*) node;
            strNodeY = (ngx_duplex_node_t*) temp;
            p = (strNodeX->data.nums < strNodeY->data.nums) ? &temp->left : &temp->right;
        }
...
/* config redis server*/
static char *
ngx_redis_conf(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_str_t                   *value;
    value = cf->args->elts;
    strncpy(redis_conf_loc, (const char *)value[1].data, value[1].len);
    printf("Redis conf : %s\n", redis_conf_loc);
    return NGX_CONF_OK;
}

/* set the minium speed */
static char *
ngx_minium_speed(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_str_t                   *value;
    ngx_uint_t                  tmp_limit;

    value = cf->args->elts;
    tmp_limit = ngx_parse_size(&value[1]);
    if (tmp_limit <= 0) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
            "Default Speed \"%V\" which used when redis connection error is invalid",
            &value[1]);
        //return NGX_CONF_ERROR;
        tmp_limit = 1024;
    }
    printf("Seted minium speed : %d\n", (int)x_limit_min_speed);

    x_limit_min_speed = tmp_limit;
    return NGX_CONF_OK;
}

static char *
ngx_rank0_speed(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_str_t                   *value;
    value = cf->args->elts;
    ...
    if (cf->args->nelts == 3)
    {
        d_rank[0] = ngx_parse_size(&value[1]);
        u_rank[0] = ngx_parse_size(&value[2]);
    }

    if (d_rank[0] < x_limit_min_speed)
        d_rank[0] = x_limit_min_speed;

    if (u_rank[0] < x_limit_min_speed)
        u_rank[0] = x_limit_min_speed;

    return NGX_CONF_OK;
}

static char *
ngx_rank1_speed(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_str_t                   *value;
    value = cf->args->elts;
    ...
    if (cf->args->nelts == 3)
    {
        d_rank[1] = ngx_parse_size(&value[1]);
        u_rank[1] = ngx_parse_size(&value[2]);
    }

    if (d_rank[1] < x_limit_min_speed)
        d_rank[1] = x_limit_min_speed;

    if (u_rank[1] < x_limit_min_speed)
        u_rank[1] = x_limit_min_speed;

    return NGX_CONF_OK;
}

static char *
ngx_rank2_speed(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_str_t                   *value;
    value = cf->args->elts;
    ...
    if (cf->args->nelts == 3)
    {
        d_rank[2] = ngx_parse_size(&value[1]);
        u_rank[2] = ngx_parse_size(&value[2]);
    }

    if (d_rank[2] < x_limit_min_speed)
        d_rank[2] = x_limit_min_speed;

    if (u_rank[2] < x_limit_min_speed)
        u_rank[2] = x_limit_min_speed;

    return NGX_CONF_OK;
}

static char *
ngx_rank3_speed(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_str_t                   *value;
    value = cf->args->elts;
    ...
    if (cf->args->nelts == 3)
    {
        d_rank[3] = ngx_parse_size(&value[1]);
        u_rank[3] = ngx_parse_size(&value[2]);
    }

    if (d_rank[3] < x_limit_min_speed)
        d_rank[3] = x_limit_min_speed;

    if (u_rank[3] < x_limit_min_speed)
        u_rank[3] = x_limit_min_speed;

    return NGX_CONF_OK;
}

static char *
ngx_redis_pwd(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_str_t                   *value;
    value = cf->args->elts;
    ...
    {
        eredis_pwd = value[1]; 
        //printf("%s %lu",eredis_pwd.data, eredis_pwd.len);
    }
...
static ngx_int_t ngx_stream_init_shm_zone(ngx_shm_zone_t *shm_zone, void *data){
    ...
    ngx_speed_control_t         *shm_control;

    shpool = (ngx_slab_pool_t *)shm_zone->shm.addr;
    shm_control = ngx_slab_alloc(shpool, sizeof *shm_control);
    if (shm_control == NULL) {
        return NGX_ERROR;
    }

    shm_control->sentinel = ngx_slab_alloc(shpool, sizeof(ngx_rbtree_node_t));
    if (shm_control->sentinel == NULL) {
        return NGX_ERROR;
    }

    shm_control->tree = ngx_slab_alloc(shpool, sizeof(ngx_rbtree_t));
    if (shm_control->tree == NULL) {
        return NGX_ERROR;
    }


    ngx_rbtree_init(shm_control->tree, shm_control->sentinel, ngx_string_rbtree_insert_value);

    shm_zone->data = shm_control;
    return NGX_OK;
}
...
void travel_rbtree(ngx_rbtree_node_t* root, ngx_rbtree_node_t *sentinel)
{
    ...
    ngx_duplex_node_t* strNode = (ngx_duplex_node_t*)root;
    printf("key = %d , str=%d\n", (int)root->key, strNode->data.nums);
